...
public class DistanceMapper {
    ...
    public DistanceMapper() throws IOException {
    ...
    public double findShortestDistance(String endId) throws IOException {
        ...
        while (!queue.isEmpty()) {
            ...
            if (currentId.equals(endId)) { // 搜索到终点
                System.out.println("距离" + endId + "到" + startId + "的最短距离为：" + shortestDistances.get(endId));
                return shortestDistances.get(endId); // 返回终点到起点的最短距离
            }

            if (shortestDistances.containsKey(currentId)) {
